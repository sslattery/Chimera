...
// Tests.
//---------------------------------------------------------------------------//
// Check that a Chimera::Assertion looks different than a
...
TEUCHOS_UNIT_TEST( Assertion, differentiation_test )
{
    ...
    catch( const Chimera::Assertion& assertion )
    {
	TEST_ASSERT( 0 );
    }
    catch( ... )
...
//---------------------------------------------------------------------------//
// Check that a Chimera::Assertion can be caught and the appropriate
...
TEUCHOS_UNIT_TEST( Assertion, message_test )
{
    ...
    try
    {
	throw Chimera::Assertion( "cond", "file", 12 );
    }
    catch( const Chimera::Assertion& assertion )
    {
	...
	"Chimera Assertion: cond, failed in file, line 12.\n" );
...
TEUCHOS_UNIT_TEST( Assertion, throw_test )
{
	...
	const int line( 12 );
	Chimera::throwAssertion( message, file, line );
...
TEUCHOS_UNIT_TEST( Assertion, precondition_test )
{
	...
	std::string true_message( "Chimera Assertion: 0, failed in" );
