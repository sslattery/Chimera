...
computations within the tolerance of the converged nonlinear residual
norms given in
...
by intertial forces is dominant. Because of this, only solutions to
...
same while the \sn{1}{5} result required an extra iteration. This is
due to the fact that the convergence of the linear residual has a
strong effect on the convergence of the nonlinear residual, much
stronger that the effect on the convergence of the eigenvalue in the
$SP_N$ computations.
...
throw of the nonlinear iteration and converge on the wrong nonlinear
solution. Although the same preconditioner was used with both the
...
effect the forcing term calculation. If they are different enough, it
...
effectivly the same residual norm tolerance and produce the same
results as provided by the
verification. 

Figure~\ref{fig:ra1e5_convergence} gives the nonlinear residual
behavior for the \sn{1}{5} Rayleigh number case. We see that the extra
iteration comes from slightly slower quadratic convergence with FANM
for this particular case. The difference in convergence of the linear
problem comes from the fact that the same multigrid preconditioner
parameters are used for this case as for the \sn{1}{3} and \sn{1}{4}
Rayleigh number cases. As the Rayleigh number increases, the problem
becomes more difficult to solve and therefore a more agressive
preconditioner should maintain iterative performance of the nonlinear
method. This was observed to be the case as for the \sn{1}{6}
Rayleigh number case the number of multigrid cycle applications was
bumped up to 3 from 1 used for the other cases and FANM converged in
the same number of nonlinear iterations as the Newton-Krylov method.
...
problem even with both solver preconditioned with the same multigrid
...
values show that FANM has effectively the same time complexity as a
...
Table~\ref{tab:driven_nonlinear_iter_comparison} show the same number
of Newton iterations were required to converge the 100, 300, and 500
Reynolds number cases while the FANM calculation with a Reynolds
number of 700 actually converged in 4 fewer iterations. The nonlinear
residual norm as a function of Newton iteration for this case is given
in Figure~\ref{fig:re700_convergence}. 
...
counts as given by Table~\ref{driven_linear_iter_comparison}. As the
opposite of what was observed for that benchmark, lid driven cavity
results show that the linear models were significantly more
...
iterations were required to converge with 64\% as many GMRES
iterations required as MCSA iterations for the 100 Reynolds number
case. For the 700 Reynolds number case, fewer MCSA iterations were
observed due to the fact that 4 fewer nonlinear iterations were
performed. The average number of linear iterations per nonlinear
iterations was still much larger for the FANM calculations. 
...
many more iterations that need for the flow problem driven by
