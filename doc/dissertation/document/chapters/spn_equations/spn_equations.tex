...
\citep{evans_simpli_2013}\footnote{The $SP_N$ derivations in this
  chapter and the associated appendices are heavily based on those
  presented by Evans in \citep{evans_simpli_2013}}, in order to gain
full understanding of the underlying system and its potential behavior
in a Monte Carlo context. From the $P_N$ equations, we apply a set of
approximations to yield the $SP_N$ equations for fixed source and
criticality problems. Using the fully-formed linear operator for the
transport problem, we explore solutions to the $SP_N$ equations with
...
observed when investigating the simpler transport systems.
...
energy groups. Finally performance is analyzed through comparison with
those same Krylov solvers in terms of both iterative performance and
CPU timing using the same set of problems utilized for the
verification.
...
communications amongst orders of magnitude more processors for
...
convergence as demonstrated by Dimov \citep{dimov_new_1998}. In this
...
\sn{2.205}{-9} was observed for reasons that remain unexplained. All
...
CPU timing for solutions to the fuel assembly problem.  This
performance comparison is necessary in order to assess the feasibility
of the method and identify areas that require further research and
development.
...
problem (and therefore the problem size). 
...
assembly criticality problem. The following are the signficant
observations and findings.

\begin{itemize}
\item MCSA can be used to solve the asymmetric system generated by the
  multigroup $SP_N$ equations
\item Simple Jacobi-based preconditioning reduces the spectral radius
  of the $SP_N$ system below unity, however, this is not sufficient
  for MCSA convergence
...
\item Advanced algebraic preconditioning strategies were applied to
  the $SP_N$ equations with ILUT chosen for subsequent investigations
\item The reduced domain approximation was required to mitigate the
  memory constraints of the explicit preconditioning strategy
\item MCSA relaxation parameters were developed an observed to enhance
  the time to solution for the fuel assembly problem
...
\item MCSA was observed to converge in fewer iterations per eigenvalue
  iteration than GMRES for the fuel assembly problem and more then
  Bi-CGStab
\item The explicit preconditioning strategy creates MCSA runtimes
  $O(100)$ larger than the production Krylov methods
\end{itemize}
