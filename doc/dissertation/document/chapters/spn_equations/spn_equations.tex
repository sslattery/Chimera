...
\cite{evans_simplified_2013}\footnote{The $SP_N$ derivations in this
  chapter and the associated appendices are heavily based on those
  presented by Evans in \cite{evans_simplified_2013}}, in order to gain
...
\begin{figure}[t!]
  ...
  \caption{\textbf{Average number of iterations required to converge
    ...
    \textit{Table~\ref{tab:spn_solver_defs} gives the description for
      each solver type presented in the legend with the Krylov solvers
      represented in black and the MCSA solvers represented in red.}}
