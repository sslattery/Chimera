...
extent by developing and applying the reduced domain
approximation. MCSA solutions for the fuel assembly problem are then
verified by comparing the solutions against production Krylov solvers
for problems with varying numbers of energy groups. Finally
...
For each row in the preconditioned system, we can then apply the
...
drop tolerance of \sn{1}{-5} and SPAINV parameters of a sparsity level
...
\item The reduced domain approximation was introduced to mitigate the
  consequences of composite linear operator generation
