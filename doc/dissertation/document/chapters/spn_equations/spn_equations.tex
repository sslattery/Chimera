...
solutions of the thermal radiation diffusion equation as presented in
...
character of the system and previous success with the thermal
radiuation difussion equation led us to try point Jacobi
preconditioning with only marginal results. From the sparsity plots we
note the block structure that ultimately arises from the multigroup
scattering matrices and their insertion into
Eq~(\ref{eq:A_block_matrix}). When full upscatter and downscatter are
used the resulting blocks are completely dense while only downscatter
gives a lower triangular scattering matrix and the block structure
shown in Figure~\ref{fig:group10ds}.
...
\end{figure}
In addition, in the limit of a block size of one, the block Jacobi
method reduces to the point Jacobi method.
...
representative of physical nuclear systems.
...
modify the source of neutrons and the multiplication factor at every
eigenvalue iteration while not affecting Monte Carlo transport.
...
pair of production Krylov solvers with which we will compare direct
numerical results in order to verify MCSA solutions in this section.
...
will be directly compared point-wise over all group and cell fluxes
computed by converging the criticality problem using both BiCGStab and
GMRES production implementations from the Trilinos scientific
computing library Aztec \cite{heroux_overview_2005}. In addition to
comparing the cell and group fluxes, the number of eigenvalue
iterations required to converge the problem along with the eigenvalue
itself will be compared as a means of verification. For the flux
values, we look to the 2-norm of the absolute difference between the
reference case and the comparison case in each energy group:
\begin{equation}
  || \ve{\Phi}_{ref}^g - \ve{\Phi}^g ||_2 < \epsilon_k\:,
  \label{eq:norm_comparison}
\end{equation}
such that this norm is less than the eigensolver tolerance,
$\epsilon_k$. For this work, the BiCGStab results will be taken as the
reference case. If the MCSA flux results satisfy
Eq~(\ref{eq:norm_comparison}), they will be deemed correct for the
purposes of this work. For the eigenvalues, we will look for the same
agreement within the tolerance of the eigensolver.
...
$\epsilon_k = \sn{1}{-6}$ and a linear solver tolerance of \sn{1}{-8}
in a serial calculation. Table~\ref{tab:serial_ev_results} gives the
eigenvalue computed by each solver for each problem as well as the
number of eigenvalue iterations required to converge. For every
problem, each solver converged to the same eigenvalue up to the number
of digits reported by the eigensolver eigensolver in the same number
of eigenvalue iterations.
\begin{table}[h!]
  \begin{center}
...
Table~\ref{tab:serial_differences_g4} give the 2-norms of the cell
scalar fluxes difference vectors for all calculations and all energy
groups. All comparisons were performed within a group such that the
...
and spatial domain are within these limits. Based on the flux results
and the eigenvalue results presented here, the MCSA results should be
deemed correct for the fuel assembly problem for serial calculations
with multiple energy groups.
...
\begin{table}[h!]
  \begin{center}
    \begin{tabular}{lccc}\hline\hline
      ...
      \multicolumn{1}{c}{\textbf{$|| \ve{\Phi}_{ref}^g - \ve{\Phi}^g ||_2$}} \\
      \hline
      GMRES-ILUT & 0 & \sn{4.562}{-6} \\
      MCSA-ILUT-R-C & 0 & \sn{2.099}{-5} \\
      MCSA-ILUT-MR-C & 0 & \sn{4.812}{-5} \\
      MCSA-ILUT-R-EV & 0 & \sn{2.371}{-5} \\
      MCSA-ILUT-MR-EV & 0 & \sn{3.605}{-5} \\
      Richardson-ILUT & 0 & \sn{1.531}{-5} \\
      \hline\hline
    \end{tabular}
  \end{center}
  \caption{\textbf{Serial scalar flux verification results for the
      1-group fuel assembly problem.} \textit{BiCGStab is used as the
      reference. Each calculation was converged with the same
      eigensolver and linear solver
      tolerances. Table~\ref{tab:spn_solver_defs} gives the
      description for each solver type presented.}}
...
\begin{table}[h!]
  \begin{center}
    \begin{tabular}{lccc}\hline\hline
      ...
      \multicolumn{1}{c}{\textbf{$|| \ve{\Phi}_{ref}^g - \ve{\Phi}^g ||_2$}} \\
      \hline
      GMRES-ILUT & 0 & \sn{7.590}{-6} \\
      MCSA-ILUT-R-C & 0 & \sn{5.308}{-6} \\
      MCSA-ILUT-MR-C & 0 & \sn{6.387}{-5} \\
      MCSA-ILUT-R-EV & 0 & \sn{4.197}{-5} \\
      MCSA-ILUT-MR-EV & 0 & \sn{2.736}{-5} \\
      Richardson-ILUT & 0 & \sn{3.976}{-6} \\
      \hline
      GMRES-ILUT & 1 & \sn{7.579}{-6} \\
      MCSA-ILUT-R-C & 1 & \sn{1.645}{-5} \\
      MCSA-ILUT-MR-C & 1 & \sn{2.634}{-4} \\
      MCSA-ILUT-R-EV & 1 & \sn{1.797}{-4} \\
      MCSA-ILUT-MR-EV & 1 & \sn{1.556}{-4} \\
      Richardson-ILUT & 1 & \sn{4.031}{-6} \\
      \hline\hline
    \end{tabular}
  \end{center}
  \caption{\textbf{Serial scalar flux verification results for the
      2-group fuel assembly problem.} \textit{BiCGStab is used as the
      reference. Each calculation was converged with the same
      eigensolver and linear solver
      tolerances. Table~\ref{tab:spn_solver_defs} gives the
      description for each solver type presented.}}
...
\begin{table}[h!]
  \begin{center}
    \begin{tabular}{lccc}\hline\hline
      ...
      \multicolumn{1}{c}{\textbf{$|| \ve{\Phi}_{ref}^g - \ve{\Phi}^g ||_2$}} \\
      \hline
      GMRES-ILUT & 0 & \sn{2.376}{-5} \\
      MCSA-ILUT-R-C & 0 & \sn{2.639}{-5} \\
      MCSA-ILUT-MR-C & 0 & \sn{3.958}{-5} \\
      MCSA-ILUT-R-EV & 0 & \sn{2.749}{-5} \\
      MCSA-ILUT-MR-EV & 0 & \sn{2.969}{-5} \\
      Richardson-ILUT & 0 & \sn{2.189}{-5} \\
      \hline
      GMRES-ILUT & 1 & \sn{2.271}{-5} \\
      MCSA-ILUT-R-C & 1 & \sn{3.702}{-5} \\
      MCSA-ILUT-MR-C & 1 & \sn{3.803}{-5} \\
      MCSA-ILUT-R-EV & 1 & \sn{4.142}{-5} \\
      MCSA-ILUT-MR-EV & 1 & \sn{2.723}{-5} \\
      Richardson-ILUT & 1 & \sn{2.137}{-5} \\
      \hline
      GMRES-ILUT & 2 & \sn{2.310}{-5} \\
      MCSA-ILUT-R-C & 2 & \sn{2.490}{-5} \\
      MCSA-ILUT-MR-C & 2 & \sn{2.214}{-5} \\
      MCSA-ILUT-R-EV & 2 & \sn{4.456}{-5} \\
      MCSA-ILUT-MR-EV & 2 & \sn{3.366}{-5} \\
      Richardson-ILUT & 2 & \sn{2.143}{-5} \\
      \hline
      GMRES-ILUT & 3 & \sn{2.335}{-5} \\
      MCSA-ILUT-R-C & 3 & \sn{3.045}{-5} \\
      MCSA-ILUT-MR-C & 3 & \sn{2.696}{-5} \\
      MCSA-ILUT-R-EV & 3 & \sn{1.946}{-4} \\
      MCSA-ILUT-MR-EV & 3 & \sn{3.295}{-5} \\
      Richardson-ILUT & 3 & \sn{2.161}{-5} \\
      \hline\hline
    \end{tabular}
  \end{center}
  \caption{\textbf{Serial scalar flux verification results for the
      4-group fuel assembly problem.} \textit{BiCGStab is used as the
      reference. Each calculation was converged with the same
      eigensolver and linear solver
      tolerances. Table~\ref{tab:spn_solver_defs} gives the
      description for each solver type presented.}}
