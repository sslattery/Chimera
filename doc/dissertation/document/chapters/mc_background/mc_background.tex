...
will not participate in the random walk and produce identical sparsity
patterns. 
...
Eqs~(\ref{direct_probability}) and (\ref{eq:direct_weight}) will
...
\begin{equation}
  ...
  \sum_{i_k}^{N} p_{i,i_1}p_{i_1,i_2}\ldots p_{i_{k-1},i_k}
  w^2_{i,i_1}w^2_{i_1,i_2}\ldots w^2_{i_{k-1},i_k} b_{i_k}^2 - x_i^2\:.
...
Often a more useful form, an alternative for Monte Carlo matrix
...
the current state the random walk starting weight will be $W_0 =
b_{i_0}$. Note here that the estimator in
...
\begin{equation}
  \sigma_j^2 = \Big(\sum_{\nu} P_{\nu} \sum_{m=0}^k W_{m}^2
  \delta_{i_m,j}\Big)_j - x_j^2\:.
  \label{eq:collision_variance_1}
\end{equation}
Expanding the transition probabilities yields the same variance as the
direct estimator, this time with the terms in reverse order and the
...
\begin{equation}
  ...
  b^2_{i_0}\delta_{i_k,j} - x_i^2\:.
  \label{eq:collision_variance_2}
\end{equation}
If we again $\ve{G} = \ve{P} \circ \ve{W} \circ \ve{W}$, we then have:
\begin{equation}
  \sigma^2_j = \sum_{k=0}^{\infty}\sum_{i_1}^{N}\sum_{i_2}^{N}\ldots
  \sum_{i_k}^{N}g_{i_{k},i_{k-1}} \ldots
  g_{i_2,i_1}g_{i_1,i_0}b_{i_0}\delta_{i_k,j}^2 - x_i^2\:,
\label{eq:collision_variance_3}
\end{equation}
which again contains the general Neumann series for $\ve{G}$ as given
by Eq~(\ref{eq:variance_neumann_series}).  Inserting $T$ back into the
variance formulation gives:
\begin{equation}
  \sigma^2_j = (\ve{T}\ve{b})_j - x_i^2\:.
  \label{eq:direct_variance_5}
\end{equation}
We can relate $\ve{G}$ to $\ve{H}$ by noting that $\ve{G}$ simply
contains an additional Hadamard product with the weight matrix. The
...
deterministically averaged at step $m$ over all potentially states $j$
...
As an example, to achieve this for diagonally dominant matrices point
Jacobi left preconditioning can be used such that the preconditioning
matrix $\ve{M}$ is:
\begin{equation}
  \ve{M} = diag(\ve{A})\:.
...
Carlo that will be solved with the Neumann Ulam method. We consider
here the general left/right preconditioned method as the left or right
preconditioned methods can be inferred from its formulation. We
consider a left preconditioner $\ve{M_L}$ and a right preconditioner
$\ve{M_R}$. The left/right preconditioned linear problem is then:
\begin{equation}
...
procedure, the \textit{composite operator},
$\ve{M}_L^{-1}\ve{A}\ve{M}_R^{-1}$, must be formed via matrix-matrix
multiplication. Several possible shortcomings of this preconditioning
approach are readily observed. First, the matrix-matrix multiplication
operation for sparse, parallel distributed matrices is significantly
more expensive than a matrix-vector multiplication operation. Second,
each preconditioner must be explicitly inverted, an operation in
itself that my be expensive prohibits the use of any preconditioners
which prohibit the extraction of the inverse. Third, for many modern
preconditioning methods, this inversion may yield dense matrices,
destroying sparsity and further impeding the performance of a
matrix-matrix multiplication operation. It is also interesting to note
that the Monte Carlo problem in the general left/right preconditioned
scheme given by Eq~(\ref{eq:left_right_correction}) is not fully
left/right preconditioned (meaning that we do not recover $\ve{x}$),
but instead part of a sequence for finding the substituted variable
...
effectively a form of variance reduction). This is due to the fact
that all states to which a stochastic history may move in a given
...
sequence as:
\begin{equation}
  p_{ij} = \frac{|h_{ji}|}{\sum_i|h_{ji}|}(1-p_{abs})\:,
  \label{eq:absorption_probabilities}
\end{equation}
and
\begin{equation}
  w_{ij} = \frac{sign(h_{ji})\sum_i|h_{ji}|}{(1-p_{abs})}\:.
...
decomposition is maintained, resulting in Monte Carlo estimates that
are unbiased.
...
method, this time with varying values of artificial
absorption. Compare CPU time and iterative performance with a fixed
grid.
