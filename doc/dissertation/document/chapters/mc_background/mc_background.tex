...
respectively vectors and $\ve{A}^T$ is the adjoint operator for
...
\begin{equation}
  ...
  g_{i_2,i_1}g_{i_1,i_0}b_{i_0}\delta_{i_k,j} - x_i^2\:,
\label{eq:collision_variance_3}
\end{equation}
where now $\ve{G}$ in this case is the transpose of that used in
Eq~(\ref{eq:direct_variance_6}). The Kronecker delta implies that the
variance contribution from each random walk will only be in its
current state and for many random walks, many starting weights of
...
Neumann series of $\ve{G}$ is now formed in reverse order in
Eq~(\ref{eq:collision_variance_3}), then
Eq~(\ref{eq:collision_variance_3}) and Eq~(\ref{eq:direct_variance_6})
are equivalent and therefore the variance of the forward and collision
estimators are equivalent as well and bound by the same restrictions
on $\rho(\ve{G})$ to ensure that the variance is finite.
...
matrix-matrix multiplication. Several possible shortcomings of this
preconditioning approach are readily observed. First, the
matrix-matrix multiplication operation for sparse, parallel
distributed matrices is significantly more expensive than a
matrix-vector multiplication operation. Second, each preconditioner
must be explicitly inverted, an operation in itself that may be
expensive and which prohibits the use of any preconditioners which
provide no mechanism to extract their inverse. Third, for many modern
...
performance with both the collision and expected value estimators.
