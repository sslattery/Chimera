...
in the random walk $j$ by sampling column-wise (or row-wise if an
...
available is available due to the work of Okten
\citep{okten_solving_2005} that uses the method of expected values as
a means to improve the Monte Carlo estimate. As outlined by Spanier
and Gelbard \citep{spanier_monte_1969}, the method of expected values
is a deterministic averaging of events that may potentially occur in
the Monte Carlo random walk sequence. Okten applied this principle
...
estimator, provides additional information at every step of the random
walk, yielding potentially better statistics with the same amount of
transport work. Conveniently, even if no Monte Carlo histories are
computed, the estimator still deterministically computes the first
