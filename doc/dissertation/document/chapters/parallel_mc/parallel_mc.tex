...
in the wall time data. First, splitting the histories amongst sets
...
case actually perfoms the fastest with only marginal improvement in
...
\begin{figure}[t!]
  ...
  \caption{\textbf{Wall time in seconds to solution for each case for
      ...
      faster to split histories amongst sets and maintain iterative
...
to solution, something that cannot be acheived with conventional
...
operations only occuring on a subset of the entire processor space of
the problem while the Monte Carlo calculation is occuring on all
processors.
...
any need to commincate histories from domain-to-domain at all. Overlap
...
Carlo may be satisfactory for accelerating the MCSA iteration.
