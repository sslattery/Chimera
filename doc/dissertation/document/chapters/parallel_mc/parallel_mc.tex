...
from the parent process in the binary tree. For this algorithm, being
mutually exclusive means that all local operations occur independently
of other processes in the system and their current state. For each
...
terminated by the weight cutoff the \textit{historiesCompleted} tally
is updated and if it hits the boundary of the domain it is added to
the outgoing history buffer for the neighboring dokmain. If that
buffer is full, it is sent to the neighboring domain with a
nonblocking operation and the memory associated with that buffer
reallocated. In all instances that
Algorithm~\ref{alg:local_history_transport} is executed,
\textit{localProcessed} is incremented as a measure of the number of
