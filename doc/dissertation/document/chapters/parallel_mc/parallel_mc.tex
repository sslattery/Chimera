...
\begin{algorithm}[h!]
  ...
  \begin{algorithmic}[1]
    ...
    \State post nonblocking receive for \textit{historiesCompleted} history tally
    \EndFor
    ...
    \EndIf
    \If{checkPeriod == \textit{localProcessed} || no local histories}
...
\begin{figure}[htpb!]
  ...
  \end{center}
  \caption{\textbf{Pure domain decomposition absolute strong scaling.}
    \textit{Super-linear speed-up from memory thrashing in base
      case. MCLS is an order of magnitude slower arithmetically,
      causing the higher efficiency.}}
...
\begin{figure}[htpb!]
  ...
  \end{center}
  \caption{\textbf{Pure domain decomposition absolute weak scaling}}
...
\begin{figure}[htpb!]
  ...
  \end{center}
  \caption{\textbf{Pure domain decomposition implementation weak
      scaling}}
