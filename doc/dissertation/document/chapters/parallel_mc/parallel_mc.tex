...
processes will be addressed at a high level along with how the Monte
Carlo data structures they require are constructed in parallel.
...
operator. Given an input decomposition for the linear operator, by
definition the Neumann-Ulam decomposition will have the same parallel
...
computed locally for this particular boundary transition. For the
parallel source, these adjacent states are not required as histories
may only be born in states in the local domain. Once the local
Neumann-Ulam decomposition has been generated with the proper data
collected from adjacent domains, Monte Carlo transport may proceed in
parallel.
...
domains, and the completion of transport. For each of these tasks,
additional algorithms, shown in bold in
...
\subsubsection{Ending the Transport Sequence}
