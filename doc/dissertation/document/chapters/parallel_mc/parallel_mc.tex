...
Neumann-Ulam method and MCSA and verify its correctness by comparing
the numerical results against production Krylov methods. We then
perform parallel scaling studings to test its performance on both a
small computing cluster and a leadership class machine and compare
this performance to the production Krylov methods.
...
the adjoint Monte Carlo algorithm must perform random walks over a
domain that is decomposed and must remain decomposed due to memory
...
%%---------------------------------------------------------------------------%%
\section{An Analytic Performance Framework for Domain-Decomposed Monte Carlo}
