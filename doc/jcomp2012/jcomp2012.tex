...
\begin{frontmatter}
  ...
  \begin{abstract}
    ...
    solving the 1T thermal radiation diffusion equation in three
    dimensions. The algorithm performance is compared against
    traditional solution schemes using a Marshak benchmark problem and
    a more sophisticated multiple material problem. Our results show
    that not only can our Monte Carlo method be an effective
    solver for sparse matrix systems, but also that it is competitive
...
extending our method to three dimensions, we have discovered that the Sequential
Monte Carlo method is actually a variant of iterative refinement cast
as a residual method.  Using Monte Carlo to accelerate a fixed-point
...
convergence over regular Monte Carlo.  Nonetheless, this method has
not gained widespread use in computational physics.  In the next
...
\end{equation}
When the spectral radius of the iteration matrix ($\vH$) is less than
1, we can expand $\vA^{-1}$ using the \latin{Neumann Series},
\begin{equation}
...
iteration\footnote{Although, a subspace method could be used instead of
  a stationary method here.}.  The residual is calculated and is used
...
The MCSA method defined in Eq.(~\ref{eq:MCSA-residual_solve}) uses the
adjoint method to estimate the error instead of the direct method
...
requried to converge is approximately the same and therefore the extra
...
As an additional comparision, the convergence behavior of MCSA can be
...
\begin{figure}[h!]
  ...
  \caption{ Two-dimensional plot of the temperature at 100~sh on a
...
\begin{figure}[h!]
  \begin{center}
    \begin{tabular}{cc}
      \subfigure[Edit
        points]{\includegraphics[width=2in,clip]{points.pdf}} &
