...
\begin{figure}[h!]
  \centering
  \includegraphics[width=5in,clip]{dir_adj_convergence.pdf}
...
higher than that computed by Halton's method. Even with the doubling
of the number of stochastic histories computed per time step in order
to ensure convergence for the Sequential method, we still see
robustness issues with a non-monotonically decreasing residual
observed in both cases and more pronounced for the $N=100$ case. In
both cases the MCSA solver is observed to be robust with a
