...
\begin{frontmatter}
  ...
  \address[ornl]{Oak Ridge National Laboratory, 
    1 Bethel Valley Rd., Oak Ridge, TN 37831 U.S.A.}
  ...
  \begin{abstract}
    ...
    solving the 1T thermal radiation diffusion equation in 3
    dimensions. We show that this method can be an effective solver
    for sparse matrix systems and is competitive with deterministic
    methods including preconditioned Conjugate Gradient. We also
    discuss various aspects of preconditioning and its general
...
\S~\ref{sec:discr-form-radi}.  We discuss using Monte Carlo
...
with $k$ events (particles in the context of radiative transfer) that
initiates in state $i$,
...
contributions to each component of $X$ are generated only in the
starting state of the particle, $i_0$, by the source term in the
current state, $b_{i_m}$. Therefore, for a given random walk
permutation, each state must be used as a starting value in order to
calculate its component of the expectation value (i.e. a state of size
$N$ will have $N$ particles per random walk permutation). When the
particle transitions between states, for example $i\rightarrow j$, the
particle weight is multiplied by the transition factor.
...
$(\vI-\vA) \delta \vx^{l+1}$ from Eq.~(\ref{eq:synthetic_error}),
giving a linear system to solve for the error:
...
fixed-point iteration\footnote{Although, a Krylov method could be used
  instead of a stationary method here.}.  The residual is calculated
and is used as a source to estimate the error, $\delta\vx^{l+1/2}$, by
...
solver. All computations presented in this section were completed on a
2.66 GHz Intel Core i7 machine with 8 GB 1067 MHz DDR3 memory
...
compute $N \times N$ random walks per permuation while the adjoint
solver will only compute one. Initially, this may not seem like a fair
comparison, however, we see from Fig.~\ref{fig:poisson_iterations}
...
random walk sequence contributes to the state in which it currently
...
number of random walks per iteration is in fact larger for the direct
method, the solution computed at each iteration is no better than that
...
\begin{figure}[h!]
  \centerline{
    \input{3D_mesh_segment.pdftex_t}}
...
\begin{figure}[h!]
  \begin{center}
    \begin{tabular}{cc}
      \subfigure[Edit points]{\includegraphics[width=2in,clip]{points.pdf}} &
      \subfigure[Temperature]{\includegraphics[width=4in,clip]{multi_mat.pdf}}
    \end{tabular}
  \end{center}
