...
two-dimensional time-dependent heat equation as a simple model
problem
...
Laplacian is differenced on a square Cartesian grid with a
...
to converge to a solution, a problem of constant grid size and
$\alpha$ was used with varying values of problem size, fixing the
...
both the direct and adjoint solvers. For each case, 50 histories were
simulated per cycle with a weight cutoff of $1.0E-4$ for the
stochastic solver and a convergence tolerance of $1.0E-8$ for the MCSA
solver. All computations were completed on a 2.66 GHz Intel Core i7
machine with 8 GB 1067 MHz DDR3 memory.

Fig.~\ref{fig:hmcsa_cpu_time} gives the CPU time needed for each case
to converge in seconds and Fig.~\ref{fig:hmcsa_iterations} gives the
number of iterations needed for each case to converge. We see clearly
in Fig.~\ref{fig:hmcsa_cpu_time} that the using the adjoint solver
...
fair comparison, however, we see from Fig.~\ref{fig:hmcsa_cpu_time}
...
\begin{figure}[htbp!]
  ...
  \caption{\sl Iterations to converge vs. Problem Size (N for an NxN
    ...
    iteration behavior.}
  \label{fig:hmcsa_iterations}
\end{figure}

Although CPU time is shown to be greatly decreased by using the
adjoint solver in Fig.~\ref{fig:hmcsa_cpu_time}, the iteration
...
size. Fig.~\ref{fig:hmcsa_convergence} gives the results of these
...
\begin{table}[htpb!]
  ...
  \label{tab:multimat_comparision}
\end{table}
