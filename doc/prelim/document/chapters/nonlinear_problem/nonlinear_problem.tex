...
automatically by evaluating it. Automatic differntiation is built on
...
The work of Bartlett and others \citep{hutchison_automatic_2006}
...
researched automatic differentation are measuring its performance
...
generating the Jacobian matrix. However, as we observed in previous
sections, these matrix-free methods suffer from poorly scaled problems
and the first order error introduced by the Jacobian approximation. In
addition, it was observed that eventually, the savings induced by the
matrix-free approach is eventually amortized over a number of
nonlinear iterations where it becomes more efficient computationally
...
maintaining an robust an efficient parallel implementation.
...
he provides us with space and time complexities for these
...
formed matrix case. For larger sparse matrices, this disparity in
memory requirements will be even greater, especially for
ill-conditioned systems that require many GMRES iterations to
converge.
...
formulation will have different parallel performance than these
methods, and therefore we should compare FANM perfomance to JFNK-based
schemes. More important than performance in the situation is the
