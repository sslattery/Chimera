...
\begin{frame}{Generalization of MCSA for Linear Problems}
  ...
  \begin{itemize}
  \item Work to date has used a physics-based formulation
    \begin{itemize}
    \item Radiation transport equations used as model system
    \item Transition probabilities built from problem-specific
      parameters
    \item Probabilities and weights must be rederived for each new
      equation set
    \end{itemize}
...
\begin{frame}{Monte Carlo Solution Methods for Nonlinear Problems}
  ...
  \item Segregated methods lack the consitency of fully implicit
    methods
...
\begin{frame}{Proposed Numerical Experiments}
  ...
  \item Performance for assymmetric systems
...
\begin{frame}{Proposed Challenge Problem}
  ...
  \begin{itemize}
  \item Problems of interest are large scale problems in reactor
    physics
  \item CASL has leveraged the Drekar multiphysics code
  \item Coupled fluid flow and heat transfer is required to
    characterize many phenomena
    \begin{itemize}
