...
\begin{frame}{Newton-Krylov Methods}
  ...
  \item GMRES with a long recurrence relation obeserved as more robust
  \item Generates a monotonically decreasing residual from maintaining
    the optimization and orthogonality conditions
  \end{itemize}
  ...
  \item The Jacobian can come from hand-coded derivatives
    ...
    \item Repeated for each equation set and hard to do for multiphysics
    \end{itemize}
  \end{itemize} 
...
\begin{frame}[fragile]{The FANM Method}
  ...
  \begin{algorithm}[H]
    \begin{algorithmic}[1]
      \STATE $k := 0$
      \WHILE{$||\ve{F}(\ve{u}^{k})|| > \epsilon ||\ve{F}(\ve{u}^{0})||$}
      \STATE $\ve{J}(\ve{u}^{k}) \leftarrow AD(\ve{F}(\ve{u}^k))$
      \COMMENT{Automatic differentation}
...
\begin{frame}{Jacobian Storage vs. Subspace Storage}
  ...
  \begin{beamerboxesrounded}[upper=boxheadcolor,lower=boxbodycolor,shadow=true]
    ...
    \item 72 total elements required for Jacobian and probability matrix
    \end{itemize}
  \end{beamerboxesrounded}
...
\begin{frame}{Parallel FANM Method}
  ...
  \begin{itemize}
  \item Modern FAD packages are parallelized using element-level assembly
  \item This work will generate a parallel MCSA solver
  \item Application of the Newton correction requires a parallel vector
    update
