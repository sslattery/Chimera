...
Neumann-Ulam method, we must explore its behavior in a domain
...
compute nodes owning neighboring pieces of the global domain.
...
\begin{multline}
  -\frac{1}{6\Delta^2}[4 \phi_{i-1,j} + 4 \phi_{i+1,j} + 4
...
\begin{multline}
  -\frac{1}{6\Delta^2}[4 \phi_{-1,j} + 4 \phi_{1,j} + 4 \phi_{0,j-1} +
