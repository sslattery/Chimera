...
particular operator. On the left, figure~\ref{fig:measured_length}
...
between the measured and analytic results. However, we obeserve a
...
respectively with weight cutoff of \sn{1}{-4}. Looking at the
...
not affect the approximation. In general, the mean chord approximation
is a better choice to estimate the leakage fraction in a domain from
the adjoint Neumann-Ulam method and except for a single data point
with $n_i=50$, the mean chord approximation yielded leakage fractions
within 0.05 of the measured results. As the domain becomes more
optically thick (with both increasing $n_i$ and decreasing
...
the adjoint Neumann-Ulam method and could potentially be extended to
...
methods that leverage the adjoint method and serve as an intial
