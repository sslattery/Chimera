...
histories \cite{alexandrov_efficient_1998}. In order to enable higher
fidelity reactor physics Monte Carlo simulations, domain decomposition
has been identified as a key principle in moving forward in high
...
\end{equation}
such that we expect to select a new state, $j$, from the current
state, $i$, in the random walk by sampling the $i^{th}$ row of the
probablity matrix as a discrete distribution.  Using our result from
...
state $i$ for a particular random walk permutation is then:
\begin{equation}
...
equate to the number of states in the $i$ (or $j$ as the problem is
symmetric) direction that a history could potentially move in a single
